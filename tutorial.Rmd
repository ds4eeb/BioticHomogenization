...
This is some setup code. Please run it now.
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

# install packages if not on your machine
...
```{r load data}
load(url("https://github.com/fishglob/FishGlob_data/raw/main/outputs/Cleaned_data/NEUS_std_clean.RData"))
#load(url("https://github.com/fishglob/FishGlob_data/raw/main/outputs/Cleaned_data/GMEX_std_clean.RData"))
```
...
This code will provide a summary of the dataset columns and a small look at what is in each one. The "survey" column, for example, provides the survey name. The "source" column provides the name of the institution that performed the survey. The "year", "month", and "day" columns provide the date of a particular survey tow, etc.
```{r view}
str(data)
```

For a description of all the columns, look at the "readme" object that is provided with each FISHGLOB survey file:
```{r metadata}
View(readme)
```
...
# plot(data$longitude, data$latitude, col = is.na(data$flag_trimming_hex8_0)+1, xlab = "Longitude", ylab = "Latitude")
# plot(data$longitude, data$latitude, col = is.na(data$flag_trimming_hex8_2)+1, xlab = "Longitude", ylab = "Latitude")
# plot(data$longitude, data$latitude, col = is.na(data$flag_trimming_hex7_0)+1, xlab = "Longitude", ylab = "Latitude")
# plot(data$longitude, data$latitude, col = is.na(data$flag_trimming_hex7_2)+1, xlab = "Longitude", ylab = "Latitude")
# ggplot(data, aes(longitude, latitude)) +
#     geom_point(color = is.na(data$flag_trimming_hex8_2)+1) +
#     facet_wrap(vars(year))

```

## 4. Clean up dataset
We need to clean up the dataset to prepare for the analysis of biotic homogenization.

First, we will remove the observations not resolved to species or subspecies. The "rank" column provides a handy indication of the taxonomic rank of each entry.
```{r spptrim}
datatrim <- data[data$rank %in% c("Species", "Subspecies"),]
```

Second, we will trim every year to a consistent spatial footprint (see Fig. 3 in Maureaud et al. 2024 https://doi.org/10.1038/s41597-023-02866-w for a visual representation). The "flag_trimming" columns provide some pre-calculated options. We will use "flag_trimming_8_0", which is the finer spatial grain option and allows 0% missing years. TRUE indicates a row to remove, NA indicates a row to keep.
```{r spatialtrim}
...
Let's plot the locations of the survey tows again, for a comparison against the full dataset we mapped earlier.
```{r map}
...
We use the Jaccard dissimilarity metric among locations in each year. This takes a little while to loop through each year of the survey (a few seconds).
```{r jaccard}
...
## 6. Plot
Time to visualize our results! We plot year on the x-axis and dissimilarity on the y-axis (down is more homogenous). We also plot a curve through the datapoints.
```{r plot}
